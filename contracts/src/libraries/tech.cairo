...
fn upgrade_component(
    ...
    if component == Names::Tech::ENERGY {
        ...
        build_time =
            shared::tech_up_time_is_seconds(cost.steel + cost.quartz, compounds.lab, game_speed);
    ...
    set!(
        world,
        PlanetTechTimer {
            planet_id, name: component, levels: quantity, time_end: time_now + build_time
        },
    );
    cost
}
...
fn complete_upgrade(world: IWorldDispatcher, planet_id: u32) {
    ...
    if queue_status.name == Names::Tech::ENERGY {
        ...
        set!(
            world,
            PlanetTechs {
                planet_id, name: Names::Tech::DIGITAL, level: techs.digital + queue_status.levels
            },
        );
        ...
        set!(
            world,
            PlanetTechs {
                planet_id, name: Names::Tech::WEAPONS, level: techs.weapons + queue_status.levels
            },
        );
        ...
        set!(
            world,
            PlanetTechs {
                planet_id, name: Names::Tech::EXOCRAFT, level: techs.exocraft + queue_status.levels
            },
        );
    }
    set!(
        world,
        PlanetTechTimer {
            planet_id, name: Zeroable::zero(), levels: Zeroable::zero(), time_end: Zeroable::zero()
        }
    );
}
